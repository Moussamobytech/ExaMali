...
const PdfViewer = () => {
  const source = { uri: 'http://192.168.1.17:5000/pdf/1', cache: true }; // Remplacez par l'URL de votre fichier PDF
    ...
    <View style={styles.container}>
      <Pdf
        ...
        onLoadComplete={(numberOfPages) => {
          ...
          Alert.alert('Succès', `Le PDF contient ${numberOfPages} pages.`);
        }}
        onError={(error) => {
          console.error(`❌ Erreur de chargement du PDF :`, error);
          Alert.alert('Erreur', 'Impossible de charger le fichier PDF.');
        }}
