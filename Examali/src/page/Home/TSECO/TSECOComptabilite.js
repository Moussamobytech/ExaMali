...
const TSECOComptabilite = () => {
  const navigation = useNavigation();
  const [searchQuery, setSearchQuery] = useState('');
  const [searchResults, setSearchResults] = useState([]);
  const [isModalVisible, setModalVisible] = useState(false);
  const [activeSubject, setActiveSubject] = useState('Comptabilité'); // Default active subject
  ...
  return (
    <View style={[styles.container, isDarkMode ? darkStyles.container : lightStyles.container]}>
      ...
      <ScrollView vertical showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContainer}>
        <Text style={[styles.subtitles, { color: dynamicTextColor }]}>Comptabilité pages TSECO</Text>
...
export default TSECOComptabilite;
